...
df = pd.read_excel('data/TREx/df_imsi.xlsx')

df['trex_one_hop'] = df['TREx_id']
...
for i in tqdm(range(len(df))):
    try:
        ans = main(df.iloc[i]['TREx_id'])
        time.sleep(2)
        df.loc[i, 'trex_one_hop'] = ','.join(ans)
        
    except:
        print(f'error occured in {i}')
        df.loc[i, 'trex_one_hop'] = 'null'
        time.sleep(2)
        

df.to_excel('df_imsi_trex.xlsx', index = False)