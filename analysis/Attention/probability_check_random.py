...
def hop_sentence_prob(model, tokenizer, data, check_col, block_opt, opt = 0):
    ...
    for i in tqdm(range(len(data)-1)):
        prefix_text = data[i+1]['generated_sentences'][check_col]['sentence_with_hop_word'][0] + data[i]['prompt'].format(data[i]['subject'])
        block_text = data[i+1][check_col][0]
        if opt == 0:
        ...
        count += prob
    logging.info(f'{check_col} with {t} : {round(count / len(data)-1, 6)}')
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename='probability_check_random.log',                     # 로그 파일 이름 지정
