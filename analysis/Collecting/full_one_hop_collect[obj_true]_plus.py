...
df = pd.read_excel('df_obj_true_full_rel+.xlsx', na_values='null')

print(df.columns)
#df['obj_one_hop'] = df['obj_id']
...
for i in tqdm(range(len(df))):
    if pd.isna(df.loc[i, 'obj_one_hop']):
        
        try:
            ans = main(df.iloc[i]['obj_id'])
            time.sleep(2)
            df.loc[i, 'obj_one_hop'] = ','.join(ans)
...
df.to_excel('df_obj_true_full_rel++.xlsx', index = False)